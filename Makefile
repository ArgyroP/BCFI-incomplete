...
prefix       := /home/iro/Desktop/riscv-tools
enable_stow  := no
...
LD            := $(CC)
LDFLAGS       :=  -Wl,--build-id=none -nostartfiles -nostdlib -static $(LDFLAGS) $(march) $(mabi) -fno-stack-protector
LIBS          := -lgcc
LINK          := $(LD) $(LDFLAGS)
...
$$($(2)_install_prog_exes) : % : %.o $$($(2)_prog_libnames)
	$(LINK) -o $$@ $$< $$($(2)_prog_libarg) $(LIBS) -Wl,--defsym=MEM_START=0x80000000,-T,$(src_dir)/$(2)/$(2).lds
